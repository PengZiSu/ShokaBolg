  ...
  <div class="app-container">
    ...
    <el-form :model="menuQuery" v-show="showSearch" :inline="true">
      <el-form-item label="菜单名称">
        <el-input
          @keyup.enter="queryMenus"
          v-model="menuQuery.keywords"
          clearable
          placeholder="请输入菜单名称"
          style="width: 200px"
        />
      ...
      <el-form-item>
        <el-button type="primary" icon="Search" @click="queryMenus"
          >搜索</el-button
        >
      </el-form-item>
    </el-form>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="plus" @click="addMenu(undefined)"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
          ...
          <el-col :span="24" v-if="menuForm.menuType !== 'B'">
            <el-form-item label="菜单图标" prop="icon">
              ...
              >
                ...
                <icon-select
                  @selected="(iconName) => (menuForm.icon = iconName)"
                />
              </el-popover>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12" :xs="24" v-if="menuForm.menuType === 'C'">
            <el-form-item prop="component">
              <template #label>
                <span>
                  <el-tooltip
                    content="访问的组件路径，如：/article/category,默认在views目录下"
                    placement="top"
                  >
                    <el-icon><question-filled /></el-icon>
                  </el-tooltip>
 ...
 */
const resetForm = () => {
  menuFormRef.value ? menuFormRef.value.clearValidate() : undefined;
  menuForm.value = {
    id: undefined,
    parentId: 0,
    menuName: "",
  ...
  menuFormRef.value.validate(async (valid) => {
    if (valid) {
      if (menuForm.value.id) {
        await updateMenu(menuForm.value).then(() => {
          modal.notifySuccess("修改成功");
          closeDialog();
          getMenuLists();
        });
      } else {
        await addMenu(menuForm.value).then(() => {
 ...
 * 获取菜单树
 */
const getMenuOptionTrees = async () => {
  await getMenuOptionTree()
    .then(({ data }) => {
      const tempMenuTree = [];
      const menuTreeItem = { id: 0, label: "顶级菜单", children: data.data };
