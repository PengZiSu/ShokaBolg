  ...
  <div class="app-container">
    ...
    >
      <el-form-item label="用户昵称">
        <el-input
          @keyup.enter="queryComments"
          v-model="commentQuery.keywords"
          style="width: 200px"
          placeholder="请输入用户昵称"
          clearable
        />
      ...
      <el-form-item label="来源">
        <el-select
          v-model="commentQuery.commentType"
          placeholder="评论来源"
          clearable
          style="width: 200px"
        >
      ...
      <el-form-item>
        <el-button type="primary" icon="Search" @click="queryComments"
          >搜索</el-button
        >
      </el-form-item>
    </el-form>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Delete"
          @click="deleteComments(undefined)"
          >批量删除
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="commentIdList.length === 0"
          icon="Check"
          @click="checkComments(undefined)"
          >批量通过
        </el-button>
      </el-col>
      <right-toolbar
        v-model:showSearch="showSearch"
        @queryTable="getCommentLists"
      ></right-toolbar>
    </el-row>
    <!-- 表格展示 -->
    <el-table
      ...
      <el-table-column label="操作" width="160" align="center">
        <template #default="scope">
          ...
          <el-button
            ...
            icon="Delete"
            link
            @click="deleteComments(scope.row.id)"
          >
            删除
          </el-button>
        </template>
    ...
    <pagination
      v-if="count > 0"
      :total="count"
      v-model:page="commentQuery.current"
      v-model:limit="commentQuery.size"
      @pagination="getCommentLists"
    />
  </div>
</template>
...
const getCommentLists = () => {
  loading.value = true;
  getCommentList(commentQuery.value).then(({ data }) => {
...
const checkComments = (id) => {
  ...
  modal
    .messageConfirm("确认通过已选中的数据项?")
    .then(() => {
      updateCommentCheck(checkDTO).then(() => {
