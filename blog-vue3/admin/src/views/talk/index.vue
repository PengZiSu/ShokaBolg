...
<template>
  <div class="app-container">
    ...
    <el-pagination
      ...
      v-model:current-page="talkQuery.current"
      v-model:page-size="talkQuery.size"
...
const handleOperation = (command) => {
  ...
  if (type === "delete") {
    modal
      ...
      .then(() => {
        deleteTalk(talkForm.value.id).then(() => {
...
// 选择说说状态
const selectTalkStatus = (command) => {
...
const addEmoji = (key, value) => {
  editorRef.value
    .addText(`<img src= \'${value}\' width=\'24\' height=\'24\' alt=\'${key}\'
...
const submitForm = () => {
  ...
  if (talkForm.value.id !== undefined) {
    updateTalk(talkForm.value).then(() => {
      uploadList.value = [];
...
const getList = () => {
  getTalkList(talkQuery.value).then(({ data }) => {
