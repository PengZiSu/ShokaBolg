      ...
      >
        <template #data>
          <div v-for="(classify, index) in classifyList" :key="index">
            ...
            >
              <n-grid-item
                v-for="(favourite, index) in classify.favouriteList"
                :key="index"
                class="favourite-item"
                @click="toUrl(favourite.url)"
              >
                <!-- 图标处理，分svg、链接和无图标 -->
                <svg
                  v-if="favourite.icon.startsWith('svg')"
                  class="favourite-item-image"
                  :viewBox="parseSVG(favourite.icon, /\/viewBox:(.*?)\//)"
                >
                  <path
                    :d="parseSVG(favourite.icon, /\/d:(.*)/)"
                    :fill="parseSVG(favourite.icon, /\/fill:(.*?)\//)"
                  />
                </svg>
                <svg-icon
                  v-else-if="favourite.icon === ''"
