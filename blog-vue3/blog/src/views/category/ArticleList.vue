...
<script setup>
import { ref } from "vue";
import { getCategoryArticleList, countCategoryArticles } from "@/api/category";
...
watch(
  ...
  () => {
    getCategoryArticleList(articleQuery.value)
      .then(({ data }) => {
...
onMounted(() => {
  countCategoryArticles(articleQuery.value)
    .then(({ data }) => {
