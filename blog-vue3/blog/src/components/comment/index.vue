...
<template>
  <div id="comment-container" v-if="!useAppStore().local">
    ...
    <div v-if="count > 0 && refresh">
      ...
      >
        <div class="comment-avatar">
          <n-avatar
            lazy
            :intersection-observer-options="{ root: '#comment-container' }"
            size="large"
            round
            :src="comment.avatar"
          />
        ...
        <div class="comment-main">
          ...
          <div class="comment-info">
            ...
            <n-button
              class="reply-btn"
              @click="commentOrReply(index, comment)"
              text
              >回复</n-button
            >
          </div>
          ...
          >
            ...
            <n-button
              text
              class="view-more-btn"
              @click="getMoreReplies(index, comment)"
              >点击查看</n-button
            >
          </div>
          <paging
    ...
    <div
      v-else
      style="padding: 1.25rem; text-align: center; color: var(--grey-5)"
    >
      暂时还没有评论，来发表第一个评论吧~
    </div>
  </div>
...
<script setup>
import { likeComment, getReplyList, getCommentList } from "@/api/comment";
...
const isLike = computed(
  () => (id) => userStore.commentLikeSet.indexOf(id) !== -1 ? "active" : ""
);
 ...
 * @param comment
 */
const getMoreReplies = (index, comment) => {
...
const getCurrentPage = (current, index, commentId) => {
  getReplyList(commentId, { current: current, size: 5 })
    .then(({ data }) => {
 ...
 * @param target 评论/回复
 */
const commentOrReply = (index, target) => {
  // 隐藏其它评论下方的回复框
 ...
 */
const getList = () => {
  getCommentList(commentQuery.value)
    .then(({ data }) => {
